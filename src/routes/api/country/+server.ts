...
export const PUT: RequestHandler = async ({ request }) => {
    ...
    const { error: err } = await supabase.from('countries').update({ point_count: newPoint_count }).match({ user_id: user_id, country: country });
...
export const DELETE: RequestHandler = async ({ request, cookies }) => {
    ...
    const { error: err } = await supabase.from('countries').delete().match({ user_id: user_id, country: country });
...
export const POST: RequestHandler = async ({ request, cookies }) => {
    ...
    const { error: err } = await supabase.from('countries').insert({ user_id: user_id, country: country, point_count: 1 });
